...
The variance of their product is calculated then (the formula and its derivation are in Appendix \@ref(apd-variance)).
Finally, we assume the normality of the distribution of the product of the estimates to retain an approximate 95% confidence interval of $\pm$ 2 standard deviations.
...
The estimated number of species per squared kilometer, $c$, is respectively `r c_paracou %>% round(0)`, `r c_PSE %>% round(0)` and `r c_NOU %>% round(0)` in Paracou, Piste de St Elie and Nouragues.
...
This is of course restricted by to the huge resources needed to settle a single one: three replicates are an exceptional amount of data.
Paracou, Piste de St Elie and Nouragues represent quite well the variability of local richness of the forest of French Guiana.
We made a strict selection of the data to count the numbers of species, which are thus lower bounds.
Ongoing efforts of botanists may increase the value of $c$, implying a proportional increase in the estimation of the number of species.
...
## Variance of the product of two random variables {#apd-variance}
