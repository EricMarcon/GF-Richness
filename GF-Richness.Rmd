...
If $z$ is known, the inventory of a reasonably large area $a$ allow computing $c=a^z/S(a)$.
...
It contains 12 6.25-ha at one 16-ha plots of primary rainforest summing up to a compact 91-ha inventory that can be considered continuous at the scale of French Guiana (8 million hectares).
...
We take into account the 68 one-hectare plots of the network. 
They are located in 21 locations that allow a quite good coverage of the variability of the forest in French Guiana (**map here**).
The number of plots varies across locations so the estimation of $z$ must be made with care.
We sampled one random plot at each location to obtain $21*20/2=210$ pairs of plots.
...
We repeated these steps 100 times to obtain a distribution of estimated $z$ values depending on the plots drawn in each location.
...
sim_z %>% density %>% plot
entropart::as.SimTest(mean(sim_z), sim_z) %>% autoplot
```

## Extrapolation

```{r}
# Extrapolation de Paracou : .625km², 604 sp > c=631. A mettre à jour.
paracou_area <- .625
paracou_S <- 604
(c <- paracou_S/paracou_area^z)
