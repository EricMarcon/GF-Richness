...
When applied to our data, aggregated in 100-km square cells (appendix \@ref(apd-npe)), the estimation is similar to that obtained directly from the abundance data of the plots. 
...
For the same reasons, hyperdominance is less pronounced: 4% of the species contain half the trees (appendix \@ref(apd-hyperdominance)), compared to 1.4% in Amazonia as a whole [@TerSteege2013].
...
The number of species estimated from Paracou, Piste de Saint-Elie and Nouragues starting points (their number of species and area) is on average close to 2800, and over 3000 when extrapolating from an average Guyadiv 1-ha plot (appendix \@ref(apd-harte2009)).
...
The Chao2 estimator if finally applied: it combines the the number of species observed in only one or two cells to estimate the number of unobserved species.
...
To be consistent with the model, its logarithm equals the average log-number of trees in all 1-ha plots. We retain `r round(Guyadiv_n_ha)` trees/ha.

Initial inventories, e.g. `r paracou_S` trees in `r paracou_envelope*100` ha in Paracou (see the Methods section) and the (log-) average number of species in Guyadiv plots, are the starting points of the estimation.
...
In all cases, the estimated values 
...
The estimated number of species is obtained for $n$ equal to 8 Mha times `r round(Guyadiv_n_ha)` trees/ha:
