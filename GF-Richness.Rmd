...
In French Guiana, the Guyadiv network consists of xxx plots accross the whole forest.
...
The methods used in these studies are not appropriate to estimate regional diversity, i.e. at a smaller scale where dispersal limitation is critical.
The contibution of this paper is to estimate the number of tree species at the regional scale, in French Guiana (8 million hectares of tropical moist forest with no ecological boundary to distinguish them from the rest of Amazonia) and demonstrate which method is valid to do so.
